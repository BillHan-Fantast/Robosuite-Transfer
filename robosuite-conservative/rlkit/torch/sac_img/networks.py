...
class Actor(nn.Module):
    ...
    def __init__(self, image_obs_shape, state_shape, action_shape, feature_dim, hidden_dim,
        ...
        if use_state_input:
            latent_dim += state_shape

        self.log_std_bounds = log_std_bounds
        self.trunk = utils.mlp(latent_dim, hidden_dim, 2 * action_shape, hidden_depth)
    ...
    def get_action(self, obs, **kwargs):
        image_obs = ptu.from_numpy(obs['image_obs'][None])
        robot_state = ptu.from_numpy(obs['robot_state'][None])
        action = self.forward(image_obs, robot_state).sample()

        assert action.ndim == 2 and action.shape[0] == 1
        return ptu.get_numpy(action[0]), {}
...
class CustomMakeDeterministic(object):
...
class Critic(nn.Module):
    ...
    def __init__(self, image_obs_shape, state_shape, action_shape, feature_dim,
        ...
        latent_dim = self.encoder.feature_dim + action_shape
        if use_state_input:
            latent_dim += state_shape
    ...
    def forward(self, obs, state, action, detach_encoder=False):
        assert obs.size(0) == action.size(0) and obs.size(0) == state.size(0)
        obs = self.encoder(obs, detach=detach_encoder)
