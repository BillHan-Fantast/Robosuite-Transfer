...
class ImageDecoder(nn.Module):
    def __init__(self, depth=32, stride=2, activation=nn.ReLU, embed_size=1024, shape=(3, 64, 64)):
        ...
        conv5_shape = conv_out_shape(conv4_shape, padding, conv5_kernel_size, stride)
        conv5_pad = output_padding_shape(conv4_shape, conv5_shape, padding, conv5_kernel_size, stride)
        self.conv_shape = (32 * depth, *conv5_shape)
        self.linear = nn.Linear(embed_size, 32 * depth * np.prod(conv5_shape).item())
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(32 * depth, 8 * depth, conv5_kernel_size, stride, output_padding=conv5_pad),
            activation(),
            nn.ConvTranspose2d(8 * depth, 4 * depth, conv4_kernel_size, stride, output_padding=conv4_pad),
            activation(),
