...
class GymVisualCatWrapper(Wrapper, Env):
    """
    ...
    def __init__(self, env, state_keys=None, visual_keys=None, object_keys=None, image_process_kwargs={}):
        ...
        self.image_obs_size = image_process_kwargs['image_obs_size']
        self.object_cat_lst = image_process_kwargs['object_cat_lst']
        self.state_cat_lst = image_process_kwargs['state_cat_lst']
    ...
    def _process_obs(self, obs_dict, verbose=False):
        ...
        state_lst = self._process_state(state_lst)
        object_lst = self._process_object(object_lst)

        obs = OrderedDict()
        obs["robot_states"] = state_lst
        obs["object_states"] = object_lst
        obs["image_obses"] = visual_lst
        obs["visual_images"] = image_lst
    ...
    def _process_state(self, state):
        cat_lst = []
        for slice in self.state_cat_lst:
            cat_lst.append(state[..., slice[0]:slice[1]])

        return np.concatenate(cat_lst, axis=-1)

    def _process_object(self, object):
        cat_lst = []
        for slice in self.object_cat_lst:
            cat_lst.append(object[..., slice[0]:slice[1]])
