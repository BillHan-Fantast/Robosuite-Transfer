...
class Push(SingleArmEnv):
    ...
    def reward(self, action=None):
        ...
        Sparse un-normalized reward:
            ...
            - a discrete reward of 2.0 is provided if the cube is pushed to the right place

        Un-normalized summed components if using reward shaping:

            - Reaching: in [0, 1], to encourage the arm to reach the cube
            - Pushing: in [0, 1], to encourage the arm to push the cube forward
        ...
        # use a shaping reward
            ...
            distance = np.linalg.norm(cube_pos - np.array([0.2, 0, table_height]))
            pushing_reward = 1 - np.tanh(5.0 * distance)
    ...
    def _check_success(self):
        ...
        cube_pos = self.sim.data.body_xpos[self.cube_body_id]
        table_height = self.model.mujoco_arena.table_offset[2]
        distance = np.linalg.norm(cube_pos - np.array([0.2, 0.0, table_height]))
