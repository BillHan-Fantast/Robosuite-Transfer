...
from models.utils.collections import (NamedArrayTuple, namedarraytuple_like,
...
def torchify_buffer(buffer_):
    ...
    if isinstance(buffer_, np.ndarray):
        return torch.from_numpy(buffer_).to(torch.float32)
...
def numpify_buffer(buffer_):
    ...
    if isinstance(buffer_, torch.Tensor):
        return buffer_.detach().cpu().numpy()
...
def buffer_to(buffer_, device=None):
    ...
    if isinstance(buffer_, torch.Tensor):
        ...
        return torch.from_numpy(buffer_).to(torch.float32).to(device)
    elif isinstance(buffer_, dict):
        for k, v in buffer_.items():
            buffer_[k] = buffer_to(v, device=device)
        return buffer_
...
def buffer_cat(buffer_A, buffer_B, dim=0, *args, **kwargs):
    buffer_ = {}
    assert buffer_A.keys() == buffer_B.keys()
    for k, va in buffer_A.items():
        v = torch.cat([va, buffer_B[k]], dim=dim)
        buffer_[k] = v
    return buffer_
