...
from models.utils.buffer import buffer_method
import numpy as np
...
class RSSMTransition(TransitionBase):
    def __init__(self, action_size, num_models, stochastic_size=30, deterministic_size=200, hidden_size=200,
        ...
        self._stochastic_prior_model_ensemble = nn.ModuleList(
            [self._build_stochastic_model() for _ in range(num_models)])
    ...
    def forward(self, prev_action: torch.Tensor, prev_state: RSSMState, k=None, sample=True):
        rnn_input = self._rnn_input_model(torch.cat([prev_state.stoch, prev_action], dim=-1))
        deter_state = self._cell(rnn_input, prev_state.deter)
        if k is None:
            k = np.random.randint(self._num_models)
        stochastic_prior_model = self._stochastic_prior_model_ensemble[k]
        mean, std = torch.chunk(stochastic_prior_model(deter_state), 2, dim=-1)
        std = tf.softplus(std) + 0.1
        dist = self._dist(mean, std)
        if sample:
            stoch_state = dist.rsample()
        else:
            stoch_state = dist.mean
...
class RSSMRepresentation(RepresentationBase):
    ...
    def forward(self, obs_embed: torch.Tensor, prev_action: torch.Tensor, prev_state: RSSMState, sample=True):
        prior_state = self._transition_model(prev_action, prev_state, sample=sample)
...
class RSSMRollout(RollOutModule):
    ...
    def rollout_representation(self, steps: int, obs_embed: torch.Tensor, action: torch.Tensor,
                               prev_state: RSSMState, sample=True):
        """
        ...
        for t in range(steps):
            prior_state, posterior_state = self.representation_model(obs_embed[t], action[t],
                                                                     prev_state, sample=sample)
    ...
    def rollout_transition(self, steps: int, action: torch.Tensor, prev_state: RSSMState, sample=True):
        """
        ...
        state = prev_state
        for t in range(steps):
            state = self.transition_model(action[t], state, sample=sample)
            priors.append(state)
    ...
    def rollout_policy(self, steps: int, policy, prev_state: RSSMState):
        ...
        for t in range(steps):
            action = policy(buffer_method(state, 'detach'))
