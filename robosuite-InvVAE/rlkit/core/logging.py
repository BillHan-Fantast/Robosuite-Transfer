...
class Logger(object):
    ...
    def dump_tabular(self, file, *args, **kwargs):
    ...
    def save_itr_params(self, itr, params, suffix=''):
        if self._snapshot_dir:
            if self._snapshot_mode == 'all':
                file_name = osp.join(self._snapshot_dir, 'itr_%d.pkl'+suffix % itr)
                torch.save(params, file_name)
            elif self._snapshot_mode == 'last':
                # override previous params
                file_name = osp.join(self._snapshot_dir, 'params.pkl'+suffix)
                torch.save(params, file_name)
    ...
    def load_itr_params(self, itr):
        if self._snapshot_dir:
            if self._snapshot_mode == 'all':
                ...
                return torch.load(file_name, map_location='cuda')
            elif self._snapshot_mode == 'last':
                # override previous params
    ...
    def atomic_replace(self, file_names):
        if self._snapshot_dir:
            ...
            for file_name in file_names:
                temp_file_name = osp.abspath(osp.join(self._snapshot_dir, file_name))
                assert osp.exists(temp_file_name)
                assert '.tmp' in file_name
                os.replace(
                    temp_file_name,
                    temp_file_name.replace('.tmp', '')
                )

    def move_files(self, src_dir, trg_dir, file_names):
        assert isinstance(file_names, list)
        for file_name in file_names:
            src_file_name = osp.abspath(osp.join(src_dir, file_name))
            trg_file_name = osp.abspath(osp.join(trg_dir, file_name))
            os.replace(
                src_file_name,
                trg_file_name
            )

    def save_model_params(self, params, epoch):
        if self._snapshot_dir:
            file_name = osp.join(self._snapshot_dir, 'model_'+str(epoch)+'.pkl')
            torch.save(params, file_name)
