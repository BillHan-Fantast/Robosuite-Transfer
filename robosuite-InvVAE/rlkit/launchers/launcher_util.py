...
def setup_logger(
        ...
        tabular_log_files=list("progress.csv"),
        snapshot_mode="last",
        snapshot_gap=1,
    ...
    first_time = not checkpoint

    if checkpoint and not osp.exists(osp.join(log_dir, 'params.pkl')):
        checkpoint = False
        first_time = True
        logger.log("Checkpoint directory does not exist. Use new log directory.")

    if checkpoint:
        # Sanity check of Checkpoint Variants
        variant_log_path = osp.join(log_dir, variant_log_file)
        with open(variant_log_path, 'rb') as f:
            old_variant = json.load(f)
            assert variant == old_variant
    ...
    tabular_log_paths = [osp.join(log_dir, file) for file in tabular_log_files]
    text_log_path = osp.join(log_dir, text_log_file)

    logger.add_text_output(text_log_path)
    if first_time:
        for log_path in tabular_log_paths:
            logger.add_tabular_output(log_path)
    else:
        for log_path in tabular_log_paths:
            logger._add_output(log_path, logger._tabular_outputs,
    ...
    if git_infos is not None and not checkpoint:
        for (
    ...
    if script_name is not None and not checkpoint:
